...
| [I3 · TABFAIRGDT](implementation-tabfairgdt/) | 复现 · 主题 7：公平、公正与正义 | Panagiotou 等（ICDM 2025）；FairGAN（2018）；Panagiotou 等（2024）；Abroshan 等（2024）。[文献](implementation-tabfairgdt/references/README.md) | 在 UCI Adult 上比较无公平调整与两档公平强度的生成器，测量合成数据的下游预测效用、群体公平性、样本分布和生成耗时。 | 作者仓库采用 MIT 许可，提供示例及实验脚本；Adult 数据由加载器获取；该组设置可使用 CPU。 |
