...
作者 ZIP 覆盖环境建立、数据下载、预处理、SVM 训练和结果汇总。`build_env.sh` 默认先删除名为 `ovo_ovr` 的 Conda 环境，再建立新环境；运行前应另选专用环境名并检查脚本。当前环境缺少 `libmultilabel`，尚未完成作者训练入口的实际运行。实施时需先记录依赖版本、数据下载地址和一份小数据的原始脚本输出，再加入训练集重采样实验。最终结果分别标明沿用作者代码的部分、修改的数据处理和新增指标。
