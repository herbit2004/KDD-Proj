...
论文重新比较 one-versus-one (OVO) 与 one-versus-rest (OVR) 的多分类分解方式，尤其关注类别不平衡时整体准确率可能掩盖少数类表现的问题。项目以核 SVM 为主要复现对象，使用作者下载清单中的 Segment（2,310 条、7 类）和 Vehicle（846 条、4 类）数据，控制类别比例、超参数与评估指标；神经网络比较列为扩展实验。
